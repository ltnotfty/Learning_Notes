...
int get_socket()
{
    ...
    int retVal = getaddrinfo( SERVER, SERVERPORT, &hints, &server_info);
    if (retVal != 0) {
    ...
    for ( p = server_info; NULL != p; p = p->ai_next ) 
    {
...
// client side library for the is_prime RPF.
